...
struct MANGOS_DLL_DECL boss_bronjahmAI : public ScriptedAI
{
    ...
    void Reset()
    {
        m_BronjahmPhase = BronjahmPhaseOne;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        switch (m_BronjahmPhase)                                 // Phase 1
        {
            case BronjahmPhaseOne:
            {
                // Switching Phase, Soulstorm is cast in SpellHitTarget
                if (m_creature->GetHealthPercent() < 30.0f)
                {
                    if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT) == CAST_OK)
                        m_BronjahmPhase = BronjahmPhaseTwo;
                }

                // Corrupt Soul
                if (m_uiCorruptSoulTimer < uiDiff)
                {
                    if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CORRUPT_SOUL, SELECT_FLAG_PLAYER))
                    {
            ...
            case BronjahmPhaseTwo:
            {
                ...
                if (m_BronjahmPhase == BronjahmPhaseTwo)
                {
